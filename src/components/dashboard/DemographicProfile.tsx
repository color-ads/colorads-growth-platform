...
export function DemographicProfile({ report, historicalReports, property }: DemographicsProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Room categories + Historical table */}
      <div className="grid grid-cols-2 gap-4">
        ...
        {/* Historical table */}
        <div className="bg-white border border-gray-100 rounded-xl p-4">
          <h4 className="text-[12px] font-medium text-gray-900 mb-3">Resumen histórico</h4>
          <div className="overflow-auto max-h-[220px]">
            <table className="w-full text-[11px]" style={{ tableLayout: 'fixed' }}>
              <thead className="sticky top-0 bg-gray-50">
                <tr>
                  <th className="text-left py-1.5 px-2 font-medium text-gray-500 w-14">Mes</th>
                  <th className="text-right py-1.5 px-2 font-medium text-gray-500">Facturación</th>
                  <th className="text-right py-1.5 px-2 font-medium text-gray-500">Reservas</th>
                  <th className="text-right py-1.5 px-2 font-medium text-gray-500">ROAS</th>
                  <th className="text-right py-1.5 px-2 font-medium text-gray-500">Clics</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {historicalReports.map((r, i) => (
                  <tr key={i} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-1.5 px-2 text-right font-medium text-gray-800">{formatCOP(r.attributable_revenue || 0)}</td>
                    <td className="py-1.5 px-2 text-right text-gray-600">{r.total_bookings?.toLocaleString()}</td>
                    <td className="py-1.5 px-2 text-right font-medium" style={{ color: (r.roas || 0) >= 7 ? '#16a34a' : (r.roas || 0) >= 5 ? '#d97706' : '#dc2626' }}>
                      {r.roas?.toFixed(1)}×
                    </td>
                    <td className="py-1.5 px-2 text-right text-gray-600">{r.total_clicks?.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="sticky bottom-0 bg-white border-t border-gray-200">
                <tr>
                  ...
                  <td className="py-1.5 px-2 text-right font-medium text-gray-900">
                    {formatCOP(historicalReports.reduce((a, r) => a + (r.attributable_revenue || 0), 0))}
                  </td>
                  <td className="py-1.5 px-2 text-right font-medium text-gray-900">
                    {historicalReports.reduce((a, r) => a + (r.total_bookings || 0), 0).toLocaleString()}
                  </td>
                  <td className="py-1.5 px-2 text-right font-medium text-gray-900">—</td>
                  <td className="py-1.5 px-2 text-right font-medium text-gray-900">
                    {historicalReports.reduce((a, r) => a + (r.total_clicks || 0), 0).toLocaleString()}
                  </td>
                </tr>
              </tfoot>
            </table>
