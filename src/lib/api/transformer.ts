...
function getLeadTimeBucket(dateCreated: string, startDate: string): string {
  const days = Math.round(
    (new Date(startDate).getTime() - new Date(dateCreated.split(' ')[0]).getTime()) / 86_400_000,
  )
...
export function transformToMetrics(
  ...
  // Noches promedio por reserva atribuible
  const attrBookings = byBookingDate.filter(r => isAttr(r) && isActive(r))
  const bookingNights = attrBookings.reduce((s, r) => s + calcNights(r.startDate, r.endDate), 0)
  ...
  for (const r of byArrival) {
    ...
    if (roomType) roomCounts.set(roomType, (roomCounts.get(roomType) ?? 0) + 1)
  }

  return {
    totalRevenue:        billingData.totalRevenue,
    attributableRevenue: billingData.totalRevenue,
    guests,
    nights,
    bookingVolume,
    bookingCount,
    ...
    avgNightsPerBooking:
      bookingCount > 0 ? Math.round((bookingNights / bookingCount) * 10) / 10 : 0,
    reservationStatus: {
