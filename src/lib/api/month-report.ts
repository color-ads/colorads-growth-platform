...
// Arma el reporte mensual desde Cloudbeds (huespedes, noches, venta x pais, tortas, etc.)
// y lo guarda en monthly_dashboard_cache. Devuelve el mismo payload que consume el dashboard.
// La facturacion NO se calcula aca: la computa el dashboard desde monthly_source_revenue.
export async function buildMonthReport(slug: string, year: number, month: number) {
  ...
  // Venta por pais: REAL atribuible, grand_total por fecha de reserva (report 34)
  ...
  // Persistir en cache (service role saltea RLS)
