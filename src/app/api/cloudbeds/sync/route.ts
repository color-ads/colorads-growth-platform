...
import { getMonthData } from '@/lib/api/cloudbeds'
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Fetch Cloudbeds data + totals para reservas atribuibles
    const cloudbedsData = await getMonthData(
      apiKey,
      year,
      month,
    ...
    const metrics = transformToMetrics(
      ...
      billingData,
      property.attributable_sources ?? [],
      cloudbedsData.bookingVolume,
      cloudbedsData.bookingCount,
    )
    ...
    return NextResponse.json({
      ...
      _meta: {
        ...
        bookingCount:  cloudbedsData.bookingCount,
      },
    })
  } catch (e: unknown) {
